...
namespace MapPin.Administration.Repositories;
    ...
    ILocalTextRegistry localTextRegistry, ITypeSource typeSource) : BaseRepository(context)
{
    ...
    public ListResponse<TranslationItem> List(TranslationListRequest request)
    {
        ...
        {
            var flat = JSON.Parse<Dictionary<string, string>>(File.ReadAllText(textsFilePath));
            foreach (var pair in flat)
            {
                customTranslations[pair.Key] = pair.Value;
